...
public class ExcelUtils {
	...
	public static void initExcel(String fileName, String[] colName) {
		...
		try {
			...
			WritableSheet sheet = workbook.createSheet("G-Sensor", 0);
	...
	public static void writeArrayToExcel(float data[][], String fileName, int startRow, int startCol) {
		if (data != null && data.length > 0) {
			...
			try {
				...
				writebook = Workbook.createWorkbook(new File(fileName),
						workbook);
				WritableSheet sheet = writebook.getSheet(0);
                Log.e("ExcelUtils", "data[0] = "+ data[0].length+"data = "+data.length);
				for (int j = 0; j < data.length; j++) {
					for (int i = 0; i < data[j].length; i++) {
						sheet.addCell(new Label(j + startCol, i + 1 + startRow, data[j][i]+"",
								arial12format));
//						Log.e("ExcelUtils", data[j][i]+"");
					}
				}
				writebook.write();
			} catch (Exception e) {
	...
	public static void writeOneArrayToExcel(long data[], String fileName, int startRow, int startCol) {
		if (data != null && data.length > 0) {
			WritableWorkbook writebook = null;
			InputStream in = null;
			try {
				...
				writebook = Workbook.createWorkbook(new File(fileName),
						workbook);
				WritableSheet sheet = writebook.getSheet(0);
				Log.e("ExcelUtils", "data = "+ data.length);
				for (int i = 0; i < data.length; i++) {
					sheet.addCell(new Label(startCol, i + 1 + startRow, data[i]+"",
							arial12format));
				}
				writebook.write();
			} catch (Exception e) {
